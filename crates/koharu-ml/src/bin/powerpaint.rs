use std::path::PathBuf;

use anyhow::Result;
use clap::Parser;
use koharu_ml::powerpaint::{PowerPaint, PowerPaintOptions, PowerPaintPaths};

#[derive(Debug, Parser)]
struct Cli {
    #[arg(short, long, value_name = "FILE")]
    input: PathBuf,

    #[arg(short, long, value_name = "FILE")]
    mask: PathBuf,

    #[arg(short, long, value_name = "FILE")]
    output: PathBuf,

    /// The GGUF written by `scripts/convert_powerpaint.py`.
    #[arg(long, value_name = "FILE")]
    model: PathBuf,

    /// Directory holding `P_ctxt.safetensors` and `P_obj.safetensors`.
    #[arg(long, value_name = "DIR")]
    embeddings_dir: PathBuf,

    #[arg(long, default_value_t = 512)]
    resolution: u32,

    #[arg(long, default_value_t = 2)]
    mask_dilation: u8,

    #[arg(long, default_value_t = 20)]
    steps: i32,

    #[arg(long, default_value_t = 7.0)]
    guidance_scale: f32,

    #[arg(long, default_value_t = 1.0)]
    strength: f32,

    #[arg(long, default_value_t = -1)]
    seed: i64,

    #[arg(long, default_value_t = false)]
    cpu: bool,
}

#[tokio::main]
async fn main() -> Result<()> {
    tracing_subscriber::fmt::init();
    let cli = Cli::parse();
    let image = image::open(cli.input)?;
    let mask = image::open(cli.mask)?.to_luma8();

    koharu_ml::init().await?;
    let model = PowerPaint::load(
        koharu_ml::device(cli.cpu),
        &PowerPaintPaths {
            model: cli.model,
            embeddings_dir: cli.embeddings_dir,
        },
    )?;
    let output = model.inference(
        &image,
        &mask,
        &PowerPaintOptions {
            resolution: cli.resolution,
            mask_dilation: cli.mask_dilation,
            num_inference_steps: cli.steps,
            guidance_scale: cli.guidance_scale,
            strength: cli.strength,
            seed: cli.seed,
        },
    )?;
    output.save(cli.output)?;
    Ok(())
}
